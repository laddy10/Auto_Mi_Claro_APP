 ...
 *
 * VERSIÓN MEJORADA:
 * ✅ Prioriza textos con números (versiones, fechas)
 * ✅ Prioriza resource-ids únicos
 * ✅ Filtra elementos vacíos o poco útiles
 */
public class PageSourceParser {
    ...
    public static List<String> extractRelevantLocators(File xmlFile, int maxLocators) {
        try {
            ...
            List<String> result = prioritizedLocators.values().stream()
                    .flatMap(List::stream)
                    .limit(maxLocators)
    ...
    /**
     * ✅ CLAVE: Calcula prioridad del localizador
    ...
    private static int calculatePriority(String locator, NamedNodeMap attributes) {
        ...
        if (text != null && !text.isEmpty()) {
            ...
            if (containsNumbers(text)) {
                ...
                System.out.println("[PageSourceParser] 🎯 Texto con números: \"" + text + "\" (prioridad: " + priority + ")");
            }
        ...
        if (resourceId != null && !resourceId.isEmpty()) {
            ...
            // IDs más específicos
            if (resourceId.contains("version") || resourceId.contains("info") ||
                    resourceId.contains("title") || resourceId.contains("label")) {
                priority += 100;
            }
        }
    ...
    private static boolean containsKeywords(String text) {
        ...
        String[] keywords = {
                ...
                "nombre", "name",
                "botón", "button",
                "aceptar", "continuar", "confirmar",
                "inicio", "home", "perfil", "profile",
                "consultar", "buscar", "search"
        };
