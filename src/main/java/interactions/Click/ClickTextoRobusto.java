...
public class ClickTextoRobusto implements Interaction {
    ...
    /**
     * Click en el borde derecho (90%) del elemento encontrado por texto.
     * Útil para listas donde la acción está a la derecha (flechas, "Ver más", etc).
     */
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        try {
            ...
            elemento = driver.findElement(
                    new MobileBy.ByAndroidUIAutomator("new UiSelector().textContains(\"" + texto + "\")")
            );
        } catch (Exception e) {
    ...
    private void clickEnCoordenadas(AppiumDriver<?> driver, WebElement elemento) {
        try {
            ...
            tap.addAction(finger.createPointerMove(Duration.ofMillis(150), PointerInput.Origin.viewport(), centerX, centerY));
