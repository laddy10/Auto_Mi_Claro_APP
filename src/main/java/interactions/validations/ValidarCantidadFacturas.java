...
public class ValidarCantidadFacturas implements Interaction {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        List<WebElement> facturas = AndroidObject.androidDriver(actor)
        ...
        if (facturas.isEmpty()) {
            List<WebElement> todasLasFacturas = AndroidObject.androidDriver(actor)
                    .findElements(AppiumBy.androidUIAutomator(
                            "new UiSelector().textContains(\"Factura\")"
                    ));
        ...
        if (facturasEncontradas != cantidadEsperada) {
            throw new AssertionError("Se esperaban " + cantidadEsperada +
                    " facturas, pero se encontraron " + facturasEncontradas);
        }
    }
}