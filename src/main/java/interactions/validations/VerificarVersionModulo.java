...
public class VerificarVersionModulo implements Interaction {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        Target LBL_VERSION_DINAMICA = Target.the("Texto de Versión Dinámica")
                .located(By.xpath("//android.widget.TextView[contains(@text, 'Ver ') or contains(@text, 'Versión ')]"));

        // Verificamos si es visible antes de intentar obtener texto
        if (!LBL_VERSION_DINAMICA.resolveFor(actor).isVisible()) {
            Serenity.recordReportData().withTitle("⚠️ Versión No Encontrada")
        ...
        if (!versionActual.equals(versionEsperada)) {
            ...
            Serenity.recordReportData().withTitle("⚠️ Cambio de Versión Detectado")
                    .andContents("Esperaba: " + versionEsperada + "\n" +
                            "Encontró: " + versionActual + "\n\n" +
                            "Por favor actualice el archivo utils.ConstantsMiniVersiones.Versiones.java con la nueva versión.");

            System.out.println("⚠️ ALERTA: La versión ha cambiado. Esperada: " + versionEsperada + " | Actual: " + versionActual);

            // Opcional: Si quieres que el reporte muestre un warning visual (estado 'PENDING' o similar), aunque Serenity no tiene un estado 'WARNING' directo, el recordReportData es lo mejor.
        } else {
            Serenity.recordReportData().withTitle("✅ Versión Correcta")
