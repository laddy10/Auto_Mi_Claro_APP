...
public class ValidateLocatorsWithOllama implements Task {
  ...
  public ValidateLocatorsWithOllama(
      String xmlFileName, String locatorsOrTexts, int maxLocatorsToSend) {
    this.xmlFileName = xmlFileName;
    this.locatorsOrTexts = locatorsOrTexts;
    this.maxLocatorsToSend = maxLocatorsToSend;
  }

  public static ValidateLocatorsWithOllama using(
      String xmlFileName, String locatorsOrTexts, int maxLocatorsToSend) {
    return Tasks.instrumented(
        ValidateLocatorsWithOllama.class, xmlFileName, locatorsOrTexts, maxLocatorsToSend);
  }
  ...
  public <T extends Actor> void performAs(T actor) {
    try {
      ...
      // Si additionally se pasan localizadores concretos en locatorsOrTexts los añadimos (evitando
      // duplicados)
      if (locatorsOrTexts != null && !locatorsOrTexts.trim().isEmpty()) {
        List<String> extras =
            List.of(locatorsOrTexts.split(",")).stream()
                .map(String::trim)
                .filter(s -> !s.isEmpty())
                .map(
                    s -> {
      ...
      if (extracted.isEmpty()) {
        recordReportData()
            .withTitle("Validación de localizadores (Ollama)")
      ...
      prompt.append(
          "Estos son los localizadores (atributo=\"valor\") extraídos del page source:\n\n");
