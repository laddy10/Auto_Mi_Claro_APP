...
public class ManejarCondicionalMedicionRed implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                ValidarTextoQueContengaX.elTextoContiene(ESTAMOS_LISTOS_INICIAR_MEDICION)
        );

        EvidenciaUtils.registrarCaptura(paso2);

        actor.attemptsTo(
                ClickTextoQueContengaX.elTextoContiene(INICIAR_TEST)
        );

        // Condicional: verificar si aparece mensaje de conectarse al plan de datos
        if (Presence.of(LBL_CONECTATE_PLAN_DATOS).answeredBy(actor)) {
