...
public class OtrosMediosDePago implements Task {
  ...
  private <T extends Actor> void ejecutarValidacionesMediosPago(T actor) {
    ...
    actor.attemptsTo(Click.on(DROPDOWN_MEDIO_PAGO), WaitFor.aTime(TIMEOUT_SHORT));
    ...
    validarMedioPago(
        ...
        "Con tarjeta Codensa",
        TarjetaCreditoDebito.validarRedireccion(),
        PASO_TARJETA_CODENSA,
        false);

    // Bancolombia (requiere doble navegación hacia atrás)
    validarMedioPago(
        actor,
        "Por medio de Bancolombia",
        Bancolombia.validarRedireccion(),
        PASO_BANCOLOMBIA,
        true);
  }

  private <T extends Actor> void validarMedioPago(
      T actor, Target botonMedio, Performable tareaValidacion, String paso, boolean esBancolombia) {
    actor.attemptsTo(Click.on(botonMedio), WaitFor.aTime(TIMEOUT_SHORT));
  ...
  private <T extends Actor> void validarMedioPago(
      T actor,
      String nombreMedio,
      Performable tareaValidacion,
      String paso,
      boolean esBancolombia) {
    actor.attemptsTo(
        ClickTextoQueContengaX.elTextoContiene(nombreMedio), WaitFor.aTime(TIMEOUT_SHORT));
