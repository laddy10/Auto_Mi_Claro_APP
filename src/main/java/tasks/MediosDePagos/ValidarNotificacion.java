...
public class ValidarNotificacion extends AndroidObject implements Task {
  ...
  public <T extends Actor> void performAs(T actor) {
    ...
    actor.attemptsTo(ClickElementByText.clickElementByText(CERRAR), WaitFor.aTime(15000));

    LeerMensaje(actor);

    if (!Presence.of(TXT_CLARO).viewedBy(actor).resolveAll().isEmpty()) {
      actor.attemptsTo(
          WaitForResponse.withText(CLARO_2), ClickElementByText.clickElementByText(CLARO_2));
      ...
      if (!lblpaqueteinstalado.isEmpty()) {
        ...
        actor.attemptsTo(WaitForResponse.withText(PERSONAS), ValidarTexto.validarTexto(PERSONAS));
        ...
        actor.attemptsTo(ValidarTextoQueContengaX.elTextoContiene(COMPRASTE_PAQUETE));
    ...
    actor.attemptsTo(Atras.irAtras(), Atras.irAtras());
  }
}
