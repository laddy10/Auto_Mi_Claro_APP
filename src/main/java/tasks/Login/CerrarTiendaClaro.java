...
/**
 * EMERGENCIA: Al abrir la app con la sesión ya iniciada, se abre la Tienda Claro ("Tecnología que
 * te transforma") sobre el home, bloqueando el flujo de los casos. Esta task detecta esa pantalla y
 * la cierra con el botón X (iv_close). NO usa navigate().back() porque en este contexto eso cierra
 * la app.
 */
public class CerrarTiendaClaro extends AndroidObject implements Task {
  ...
  public <T extends Actor> void performAs(T actor) {
    ...
    for (int intento = 1; intento <= MAX_INTENTOS && textoPresente(actor, IR_A_CINE); intento++) {

      if (textoPresente(actor, MUNDO_CLARO)) {
        actor.attemptsTo(ClickTextoQueContengaX.elTextoContiene(MUNDO_CLARO), WaitFor.aTime(1500));

      } else if (textoPresente(actor, PAGOS_Y_MAS)) {
        actor.attemptsTo(ClickTextoQueContengaX.elTextoContiene(PAGOS_Y_MAS), WaitFor.aTime(1500));
      }
    }
