...
import static utils.Constants.*; // ← aquí tienes tus textos (INICIAR_SESION, CORREO_ELECTRONICO,
...
public class LoginOrquestado implements Task {
  ...
  private <T extends Actor> void asegurarSesionCerrada(T actor) {
    ...
    boolean saludoVisible =
        !Presence.of(LBL_ENCABEZADO_USUARIO).viewedBy(actor).resolveAll().isEmpty()
  ...
  private <T extends Actor> void loginPorCorreo(T actor) {
    ...
    actor.attemptsTo(
        WaitUntil.the(BTN_OTROS_METODOS_INGRESO, WebElementStateMatchers.isVisible())
            .forNoMoreThan(10)
            .seconds(),
        Click.on(BTN_OTROS_METODOS_INGRESO));

    EvidenciaUtils.registrarCaptura("Pantalla: Bottom Sheet - Selección 'Correo electrónico'");
    actor.attemptsTo(ClickElementByText.clickElementByText(CORREO_ELECTRONICO));

    EvidenciaUtils.registrarCaptura("Pantalla: Ingresar CORREO");
    actor.attemptsTo(
        WaitUntil.the(TXT_USERNAME, WebElementStateMatchers.isEnabled())
            .forNoMoreThan(10)
            .seconds(),
    ...
    actor.attemptsTo(
        ...
        WaitUntil.the(LOADING_ESPERA_UN_MOMENTO, WebElementStateMatchers.isNotPresent())
            .forNoMoreThan(30)
            .seconds());
  }
  ...
  private <T extends Actor> void cerrarSesion(T actor) {
    ...
    actor.attemptsTo(
        ...
        WaitUntil.the(LBL_INICIAR_SESION, WebElementStateMatchers.isVisible())
            .forNoMoreThan(20)
            .seconds());
  ...
  private <T extends Actor> void bypassWelcomeBackIfPresent(T actor) {
    ...
    if (!Presence.of(LNK_INGRESAR_OTRA_CUENTA).viewedBy(actor).resolveAll().isEmpty()) {
      if (LNK_INGRESAR_OTRA_CUENTA.resolveFor(actor).isClickable()
          || LNK_INGRESAR_OTRA_CUENTA.resolveFor(actor).isEnabled()) {
    ...
    if (!Presence.of(BTN_CONTINUAR_WELCOME).viewedBy(actor).resolveAll().isEmpty()) {
      ...
      actor.attemptsTo(
          WaitUntil.the(BTN_CONTINUAR_WELCOME, WebElementStateMatchers.isVisible())
              .forNoMoreThan(5)
              .seconds());
