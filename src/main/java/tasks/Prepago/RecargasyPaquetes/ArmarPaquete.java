...
public class ArmarPaquete implements Task {
  ...
  private static final String paso3 =
      "Seleccionar duración del paquete " + user.getDuracionPaqueteArmar();
  private static final String paso4 =
      "Seleccionar cantidad datos del paquete " + user.getCantidadDatosArmar();
  ...
  public <T extends Actor> void performAs(T actor) {
    ...
    actor.attemptsTo(Click.on(DROPDOWN_DURACION), WaitForResponse.withText(ELIGE_DURACION_PAQUETE));
    ...
    actor.attemptsTo(Click.on(DROPDOWN_DATOS), WaitForResponse.withText(ELIGE_GIGAS_NECESITAS));
